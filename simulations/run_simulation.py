...
FIELDNAMES = [
    "scenario", "replication", "seed", "N", "p", "true_rho", "observed_rho", "lambda",
...
def _run(config: dict[str, Any], smoke: bool) -> list[dict[str, Any]]:
    ...
    seed_index = 0
    target_value = 0.5 if 0.5 in config["fragility_targets"] else config["fragility_targets"][0]
    target = FragilityTarget("relative", float(target_value))
                ...
                for contamination_count in config["contamination_cases"]:
                    for replication in range(replications):
                        ...
                        greedy = greedy_fragility(
                            simulated.X, edge=edge, target=target, shrinkage=fitted.shrinkage
                        )
                        ...
                        rows.append({
                            ...
                            "N": n,
                            "p": p,
                            "true_rho": simulated.partial_correlation[edge],
